...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Widget _glassCard(AuthState state) {
    return BackdropFilter(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text('Welcome back', style: AppTypography.headlineLg.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            Text(
              _isSignUp
            ...
            Center(
              child: Row(
                ...
                children: [
                  ...
                  if (!_isSignUp) Text('?', style: AppTypography.bodyMd.copyWith(color: AppColors.onSurfaceVariant)),
                  TextButton(
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: Text('Password', style: AppTypography.labelMd),
            ),
            if (!_isSignUp)
              TextButton(
                ...
                child: const Text(
                  'Forgot Password?',
                  style: TextStyle(color: AppColors.primaryContainer, fontSize: 12),
                ),
              ),
          ],
        ),
        ...
        const SizedBox(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: Color(0x9953E076), blurRadius: 8),
              ],
            ),
          ),
        ),
  ...
  Widget _buildVerify(AuthState state) {
    return ConstrainedBox(
      ...
      child: _glassVerifyCard(
        icon: Icons.mark_email_unread,
        title: 'Verify your email',
        message: '6-digit code sent to ${state.pendingEmail ?? ''}',
        buttonLabel: 'Verify Email',
        onVerify: () => ref.read(authProvider.notifier).verifyEmail(_codeCtl.text),
  ...
  Widget _buildLoginVerify(AuthState state) {
    ...
    switch (reason) {
      case 'new-device':
        message = 'We noticed a sign-in from a new device or network. A code was sent to $email.';
        break;
      case 'inactive':
        message = "You haven't signed in for a while, so we want to confirm it's you. A code was sent to $email.";
        break;
      case 'unknown-location':
        message = 'We noticed a sign-in from an unfamiliar location. A code was sent to $email.';
        break;
      default:
    ...
    return ConstrainedBox(
      ...
      child: _glassVerifyCard(
        ...
        onVerify: () => ref.read(authProvider.notifier).loginVerify(_codeCtl.text),
        onResend: () {
