...
async function seed() {
  ...
  if (parseInt(existing[0].count) > 50) {
    console.log(`[seed] Already seeded ${existing[0].count} artists, skipping`)
    process.exit(0)
  }
  ...
  const artistIds = [] // Track created user IDs for graph building
  ...
  for (const person of allPeople) {
    try {
      ...
      // Get full person details with bio and credits
  ...
  console.log(`[seed] Created ${artistIds.length} accounts. Building collaboration graph...`)
  ...
  for (let i = 0; i < artistIds.length; i++) {
    ...
    try {
      ...
      // Get unique movie IDs from cast + crew
      const movieIds = new Set()
      for (const c of credits.cast || []) movieIds.add(c.id)
      for (const c of credits.crew || []) movieIds.add(c.id)

      const movieList = [...movieIds].slice(0, 10) // max 10 movies per artist
      ...
      for (const movieId of movieList) {
        try {
          ...
          // Find collaborators among other seeded artists
          ...
          const movieTitle = movieCredits.id ? (await TMDB.get(`/movie/${movieId}`, { params: { language: 'en-US' } })).data.title : 'Unknown'
          ...
          // Skip individual movie failures
        }
      }
